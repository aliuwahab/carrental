...
class BookingPaymentInstructions extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $message->line('**Booking Created Successfully!**')
            ...
            ->line('---')
            ->line('')
            ->line('**Choose Your Payment Method:**')
            ->line('')
            ->line('**Option 1: PayPal**')
            ->line('Pay securely with PayPal using the link below.')
            ->action('Pay with PayPal', route('booking.payment', ['booking' => $this->booking->id, 'method' => 'paypal']))
            ->line('')
            ->line('**Option 2: Mobile Money**')
            ->line('Send payment to our mobile number:')
